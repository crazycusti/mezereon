...
Current status / known issues (2026-02-15)
- BIOS E820 memory map works in QEMU again (Stage2 INT 15h E820 fixed). Kernel still keeps a fallback map for broken/empty BIOS memory maps.
- IRQ0/timer triple-fault fixed by installing a kernel-local GDT early in `entry32.asm` (Stage3 GDT can be overwritten by kernel BSS).
- VBE handoff is wired; framebuffer render path currently only 4/8 bpp and still being validated.
